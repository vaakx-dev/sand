...
function mountBrowser(
  ...
  const setVisibility = async (target = browser) => {
    if (!target || target !== browser || disposed) return;
    await (instance.visibility.get() ? target.show() : target.hide()).catch(() => undefined);
  };
  ...
  const stopVisibility = instance.visibility.subscribe(() => void setVisibility());
