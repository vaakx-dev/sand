...
import { spawnText } from "./process.ts";
...
export async function registerProjects(context: HostExtensionContext): Promise<void> {
  const registry = join(context.config, "projects.json");
  await remember(registry, context.workspace);

  context.commands.register("projects.list", () => load(registry));
  context.commands.register("projects.pick", pickDirectory);
  context.commands.register("projects.add", async (params) => {
  ...
  context.commands.register("projects.clone", async (params) => {
...
async function load(path: string): Promise<Project[]> {
  try {
    const projects = JSON.parse(await readFile(path, "utf8")) as Project[];
    return projects
...
function description(path: string): Project {
