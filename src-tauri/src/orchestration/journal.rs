...
use super::{
    Event, OrchestrationError, Record, Store,
    projection::{Projection, apply, correlation},
    timestamp,
};

pub(super) fn open(path: &Path) -> Result<Store, OrchestrationError> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(OrchestrationError::Directory)?;
    }
    ...
    connection.execute_batch(
        "PRAGMA journal_mode = WAL;
         ...
         CREATE INDEX IF NOT EXISTS orchestration_events_thread
           ON orchestration_events(thread_id, sequence);
         CREATE INDEX IF NOT EXISTS orchestration_events_run
...
pub(super) fn append(store: &mut Store, record: Record) -> Result<Event, OrchestrationError> {
...
fn replay(connection: &Connection) -> Result<Projection, OrchestrationError> {
...
pub(super) fn events_for(
    ...
    let mut statement = connection.prepare(
        "SELECT sequence, id, kind, thread_id, run_id, attempt_id, created_at, payload
         FROM orchestration_events WHERE thread_id = ?1 ORDER BY sequence",
    )?;
    statement
