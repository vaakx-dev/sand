...
import { ThreadStore, threadSummary } from "./threadStore.ts";
...
export class ThreadLifecycle {
  ...
  async reorderPin(threadId: string, beforeId?: string): Promise<JsonValue> {
    ...
    for (const summary of summaries) {
      this.events.emit("orchestration.thread", { thread: summary });
    }
    return summaries;
  }
  ...
  async delete(threadId: string): Promise<JsonValue> {
    ...
    this.events.emit("orchestration.thread_deleted", { threadId });
    return true;
  }
