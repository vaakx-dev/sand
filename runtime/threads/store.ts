...
export class ThreadStore {
  ...
  find(id: string): AgentThread | undefined {
  ...
  async persist(thread: AgentThread, touchUpdated = true): Promise<void> {
    if (touchUpdated) this.touch(thread);
...
function normalizeThread(thread: AgentThread): void {
  ...
  thread.queuedTurns = Array.isArray(thread.queuedTurns) ? thread.queuedTurns : [];
  if (thread.pinned) {
